...
sample_pd = pd.read_csv(samplesheet, skiprows=range(0, data_index + 1), skip_blank_lines=True)
...
iclip = 'iCLIP'
iclip_select = sample_pd.loc[sample_pd['DataAnalysisType'] == iclip].copy()
iclip_lanes_set = iclip_select['Lane'].unique().tolist()

idx_list_to_drop = []
# check if any iCLIP rows are present and if they haven't already been collapsed
if iclip_lanes_set and len(iclip_select) != len(iclip_lanes_set):
    for lane in iclip_lanes_set:
        ...
        for index, row in iclip_select.iterrows():
            if lane == row['Lane']:
                ...
                if int(iclip_sample_num) < min_value:
                    min_value = int(iclip_sample_num)
                elif int(iclip_sample_num) > max_value:
...
sc_list = ['10X-3prime', '10X-FeatureBarcoding', '10X-Multiomics-GEX']
sc_ATAC_list = ['10X-ATAC', '10X-Multiomics-ATAC']
sc_DNA_list = ['10X-CNV']
sc_Arc_list = ['10X-Arc']
...
cellranger_ref_genome_dict = {'Homo sapiens':'GRCh38', 'Mus musculus':'mm10', 'Danio rerio':'GRCz10',
                              'Gallus gallus':'Gallus_gallus', 'Drosophila melanogaster':'Drosophila_melanogaster', 'Monodelphis domestica':'Monodelphis_domestica', 
                              'Danio rerio pre-mRNA':'GRCz10_pre_mRNA'}
...
# create new csv for just 10X-Arc samples
cellranger_10XArc_df = sample_pd[sample_pd['DataAnalysisType'].isin(sc_Arc_list)].copy()
cellranger_idx_Arclist_to_drop = cellranger_10XArc_df.index.values.tolist()
cellranger_10XArc_df['ReferenceGenome'] = cellranger_10XArc_df['ReferenceGenome'].map(cellranger_ref_genome_dict).fillna(cellranger_10XArc_df['ReferenceGenome'])
...
if len(cellranger_10XATAC_df) > 0:
    with open('tenX_samplesheet.ATACtenx.csv', 'w+') as ATACfile:
        ...
        col_list = ['Lane', 'Sample_ID', 'index', 'Sample_Project', 'ReferenceGenome', 'DataAnalysisType']
        cellranger_10XATAC_df = cellranger_10XATAC_df[col_list]
